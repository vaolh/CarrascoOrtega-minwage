...
OUTFILE="../output/graph.txt"
mkdir -p ../output

ESTIMATE_DIR="../../estimate/code"
...
cat >> "$OUTFILE" << 'DOT'
    ...
    download  [label="data/download"];
    clean     [label="data/clean"];
...
echo "    { rank=same; anchor0; download; clean; readdo; }" >> "$OUTFILE"
...
# ── Visible edges ──
echo '    download -> clean;' >> "$OUTFILE"
echo '    clean -> readdo;' >> "$OUTFILE"
