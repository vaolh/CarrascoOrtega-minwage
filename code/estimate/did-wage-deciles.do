...
forvalues d = 1/10 {
    ...
    local coeflabels `coeflabels' `d'.decile#1.zlfn#1.post "Decile `d'"
}
...
*** All — no controls
reghdfe lnw ibn.decile##(i.zlfn##i.post), absorb(ubica_geo year) vce(cluster ubica_geo)
     eststo c1
...
*** Formal — no controls
reghdfe lnw ibn.decile##(i.zlfn##i.post) if informal == 0, absorb(ubica_geo year) vce(cluster ubica_geo)
     eststo c3
     estadd local controls       "N"
     estadd local hastimefe      "Y"
     estadd local hasmunicfe     "Y"

*** Formal — with controls
...
*** Informal — no controls
reghdfe lnw ibn.decile##(i.zlfn##i.post) if informal == 1, absorb(ubica_geo year) vce(cluster ubica_geo)
     eststo c5
     estadd local controls       "N"
     estadd local hastimefe      "Y"
     estadd local hasmunicfe     "Y"

*** Informal — with controls
...
esttab c1 c2 c3 c4 c5 c6 ///
    ...
    refcat(1.decile#1.zlfn#1.post "ZLFN $\times$", nolabel) ///
    keep(`keeplist') ///
