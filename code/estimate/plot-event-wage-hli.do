...
log using "log/plot-event-wage-hli.log", replace text
...
if "`ds'" == "month" {
    ...
    * Indigenous speakers
    ...
    coefplot ///
        ...
        vertical noci ///
        xline(18.5, lcolor(red) lpattern(dash)) ///
        yline(0, lw(thin) lpattern(solid)) ///
        ytitle("Coefficient Estimate on Log Wages") ///
        xtitle("Month") ///
        legend(order(1 "Non-Indigenous Speaker" 2 "Indigenous Speaker") ///
            position(11) ring(0) col(1)) ///
        xlabel( ///
...
graph export "../../paper/figures/plot-event-wage-hli-`ds'.png", replace width(4000) height(3000)
}
